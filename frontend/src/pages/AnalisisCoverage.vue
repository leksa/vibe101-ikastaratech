  ...
  <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
      ...
      <div class="stat-card border-l-4 border-green-500">
        <p class="text-sm text-slate-500">Hijau (≥90%)</p>
        <p class="text-2xl font-bold text-green-600">{{ coverageSummary.hijau || 0 }}</p>
        ...
        <table class="w-full">
          <thead class="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th class="table-header text-right">Hijau</th>
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="row in provinsiData" :key="row.provinsi" class="hover:bg-slate-50/50 transition-colors">
              ...
              <td class="table-cell text-right">
                <span v-if="row.hijau > 0" class="text-green-600 font-medium">{{ row.hijau }}</span>
              ...
              <td class="table-cell">
                <div class="h-2 rounded-full bg-slate-100 flex overflow-hidden min-w-[100px]">
                  <div v-if="row.hijau > 0" class="h-full bg-green-500" :style="{ width: (row.hijau/row.terdata)*100 + '%' }" />
